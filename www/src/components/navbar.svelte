<script lang="ts">
	...
	import HamburgerIcon from 'virtual:icons/eva/menu-fill?width=3em&height=2em';
	import { onMount } from 'svelte';

	let menuToggle: HTMLButtonElement;
	let menuLinks: HTMLUListElement;

	onMount(() => {
		menuToggle.addEventListener('click', () => {
			if (menuLinks.style.display)
				menuLinks.style.removeProperty('display');
			else menuLinks.style.display = 'flex';
		});
	});
</script>
...
<style lang="scss">
	nav {
		@apply grid grid-cols-3 <md:(grid-cols-4) <sm:(grid-rows-[1fr,fit-content] grid-cols-2) gap-4 px-4 items-center min-h-[3rem];
	...
	.links {
		@apply list-none p-0 flex justify-center items-center gap-4 sm:justify-end <sm:hidden;
		@apply col-start-3 col-end-4 md:(col-end-5) <sm:(col-start-1 row-start-2 col-end-3);
	}
</style>
